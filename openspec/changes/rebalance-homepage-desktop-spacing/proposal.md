...
- Map section rail stops to the page's reachable scroll range so the final section can activate at the document bottom.
...
- `HomeSectionRail` receives a narrow progress-normalization correction; `HomePageView` and the rendered navigation structure remain unchanged.
