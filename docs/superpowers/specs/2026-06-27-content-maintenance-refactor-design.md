...
Repository guidance is partially current. `README.md` describes the Next.js stack and translation command, but it does not explain common profile maintenance or the full Blog frontmatter and verification workflow. `CONTRIBUTING.md` and the talk-scraping workflow still describe parts of the retired Jekyll project. A stale local `CLAUDE.md` also exists, but Git explicitly ignores it and it is not a repository document.
...
- Rewrite stale tracked contributor guidance for the current Next.js project.
...
- Homepage: `{ id, time: period, title: company + " · " + role, meta: location, description }`.
- CV: `{ id, role: company + " · " + role, detail: location + " · " + period }`.

Publication records contain `date`, `paperUrl`, and a typed `category`, plus localized `venue`, `authorship`, `title`, and `description`. Selectors produce:

- Homepage: `{ id, venue, authorship, title, description, paperUrl }`.
- CV: `{ id, title, permalink: paperUrl, venue, date, excerpt, paperUrl }`. English `excerpt` is `${authorship}.`; Chinese `excerpt` is `${authorship}。`.
- Publications archive: the CV shape plus `category`, with `excerpt` composed from the description and authorship. A shared formatter trims surrounding whitespace and removes one trailing `.`, `!`, `?`, `。`, `！`, or `？` from the description. English returns `${descriptionWithoutTerminalPunctuation}; ${lowercaseFirst(authorship)}.`. Chinese returns `${descriptionWithoutTerminalPunctuation}；${authorship}。`. These rules reproduce the current archive copy exactly.

Use a `PublicationCategory` union for the existing `conferences`, `manuscripts`, and `books` message keys. Export `publicationCategoryOrder` in that order. The homepage and CV use master record order; the publications page uses category order and preserves master order within each category.

Export focused selectors such as `getHomeEducation`, `getCvEducation`, `getHomeInternships`, `getCvInternships`, `getHomePublications`, `getCvPublications`, and `getPublicationArchive`. The config module owns these deterministic transformations; components only render their returned shapes.

This boundary means a new publication is inserted once with both language variants. It appears in master-array order on the homepage and CV; the publications archive places it under its configured category, with categories ordered by `publicationCategoryOrder` and records within that category retaining master-array order.

## Component Migration

`HomePageView.tsx` will remove its local education, internship, and paper arrays, plus the duplicated incoming education fields in `homeCopy`, and consume the localized selectors from `src/config/profile.ts`.
...
Add a focused profile-config test that checks stable ordering, unique IDs, valid categories, required URLs, both locale values, and exact selector output for the current records. Component rendering tests must iterate every localized institution, internship company/role, and publication title from the selectors and assert that each appears in the applicable homepage, CV, and publications output for both locales. This guards against record loss while avoiding a full markup snapshot.

## Blog Cache Contract

Define the cache hash contract as SHA-256 of the complete raw UTF-8 MDX source. Both hash helpers accept only a source string; remove the translation script helper's object-input branch. Exact source changes, including frontmatter and whitespace changes, must invalidate the cache.

The runtime loader already hashes the raw source. Change the translation workflow to call its existing hash function with the raw source rather than a reconstructed `{ frontmatter, body }` object. Add a regression test that imports both implementations and proves that the same representative MDX source produces the same hash, a frontmatter-only change produces a different hash in both implementations, a whitespace-only body change produces a different hash in both implementations, and object input is rejected by the script helper.
...
3. Adding a Blog post under `content/posts/<YYYY-MM-DD>-<slug>.mdx` with a minimal frontmatter example containing `title`, `date`, `language`, `excerpt`, and `tags`. The current Insights route slug comes from the filename; `permalink` is not required or used by this runtime loader.
...
- Leave the ignored, untracked local `CLAUDE.md` and the existing `.gitignore` rule unchanged. Tracking or replacing local agent instructions requires a separate explicit decision.
...
- Homepage and CV publications retain master record order. The publications page uses `publicationCategoryOrder` and retains master order within each category.
...
5. Automated rendering assertions confirm every current education, internship, and publication record appears in every applicable view for both locales.
